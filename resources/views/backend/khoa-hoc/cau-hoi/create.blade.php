...
@section('main-content')
    ...
    <h1>Tạo một câu hỏi mới</h1>
    <h2>Khoá học: <b>{{ $khoaHoc->ten_khoa_hoc }}</b></h2>
    <form method="POST" action="{{ route('khoa-hoc.cau-hoi.store', $khoaHoc) }}">
        @csrf
            ...
            <div class="card-body">
                <div class="mb-3">
                    ...
                    <input id="thu_tu" type="number" name="thu_tu"
                        class="form-control @error('thu_tu') is-invalid @enderror" value="{{ old('thu_tu') }}" required
                ...
                <div class="mb-3">
                    ...
                    <textarea id="noi_dung" name="noi_dung" cols="30" rows="5" class="form-control @error('noi_dung') is-invalid @enderror" required
                        autocomplete="noi_dung">{{ old('noi_dung') }}</textarea>
                    @error('noi_dung')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
